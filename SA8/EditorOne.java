 ...
 * @author bibaswankhadka added multidrawer
 */
...
public class EditorOne extends JFrame {	
	...
	private JComponent setupCanvas() {
		JComponent canvas = new JComponent() {
			public void paintComponent(Graphics g) {
				...
				drawSketch(g);  // Call draw sketch to draw
	...
	private void handlePress(Point p) {
		...
		if (mode == Mode.DRAW) {
			shape = new Ellipse(p.x,p.y,color);  // Create new ellipse every time a click occurs
			Point from = new Point(p.x,p.y);
			drawFrom = from;
			repaint();
		}
		if (mode == Mode.MOVE) {
		...
		if (mode == Mode.DELETE) {
			if (shape.contains(p.x, p.y)) {
				shape = null;  // Set shape to null to delete
			}
			repaint();
		}
	...
	private void handleDrag(Point p) {
		...
		if (mode == Mode.MOVE && moveFrom != null) {
			shape.moveBy(p.x - moveFrom.x,p.y-moveFrom.y);  // Move by a little amount
			moveFrom = p;
			repaint();
		}
	...
	private void handleRelease() {
		// TODO: YOUR CODE HERE
		if (mode == Mode.MOVE) {
			shape.moveBy(0, 0);  // Do not move at all
			repaint();
		}
	...
	private void drawSketch(Graphics g) {
		// TODO: YOUR CODE HERE
		if (shape != null) {  // Only draw if not null
			shape.draw(g);
		}
